...
int main() {
  ...
  vector<NestedInteger> temp{NestedInteger(4), vector<NestedInteger>{NestedInteger(6), NestedInteger(7), vector<NestedInteger>{NestedInteger(8)}}};
  nested.push_back(temp);

  std::cout << "The second list:\n";
  NestedIterator ite(nested);
  while (ite.hasNext())
    std::cout << ite.next() << " ";
  std::cout << std::endl;
}
